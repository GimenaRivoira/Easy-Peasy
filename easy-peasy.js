...
`)
		...
		case '/mas-votadas':
			...
			let suma = movies.map(peli =>{
				return peli.vote_average
			})

			let suma2 = suma.reduce((acumulador, peli)=>{
				return acumulador.vote_average + peli / movies.length
			})

			res.write(`${suma2.toFixed(2)}`)
			res.write (`\n`)
			res.write(`\n-----------------------------------`)
            let filtrar = movies.filter(elemento =>{
                if(elemento.vote_average >= 7){
					res.write (`\n`)
                    res.write(
						`${elemento.title} ${elemento.vote_average}
                        `)
                }
			})
			res.write (`\n`)
